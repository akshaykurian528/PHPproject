		...
		<div class="formBox level-login">
			<div class="box loginBox">
				...
				<form class="form" action="#" method="post">
					<div class="f_row-2">
						<input type="text" class="input-field" placeholder="UPRN" name="txtuprn" required>
					</div>
...
{
  if( !empty($_POST['txtuprn']) && !empty($_POST['txtpassword']))
			   {
				   $qry="select * from student where UPRN='{$_POST['txtuprn']}' and password='{$_POST['txtpassword']}'";
				   $res=$conn->query($qry);
                   if($res->num_rows>0)
                   {
                       $rest=$res->fetch_assoc();
                       $_SESSION["sem"]=$rest["sem"];
                       $_SESSION["UPRN"]=$_POST["txtuprn"];
