		...
		<div class="formBox level-login">
			<div class="box loginBox">
				...
				<form class="form" action="#" method="post">
					<div class="f_row-2">
						<input type="text" class="input-field" placeholder="Username" name="txtusername" required>
					</div>					
					...
					<a href="studentlogin.php" style="color:white"><br><u>Student Login<u></a> 
  ...
  if( !empty($_POST['txtusername']) && !empty($_POST['txtpassword']))
			   {
				   ...
				   $l=mysqli_query($conn,$qry);
				   while($row=mysqli_fetch_array($l))
				   {
					   $user=$row['Username'];
					   $pass=$row['password'];
					   $level=$row['type'];
                         if ($_POST['txtusername'] == $user && $_POST['txtpassword'] == $pass && $level=='1')
						  {
							  header("location:adminhome page.php");
							  $_SESSION['username']=$_POST['txtusername'];
							   $_SESSION['password']=$_POST['txtpassword'];
						  }
						  if ($_POST['txtusername'] == $user && $_POST['txtpassword'] == $pass && $level=='2')
				          {
							$_SESSION['username']=$_POST['txtusername'];
							$_SESSION['password']=$_POST['txtpassword'];
					   header("location:staffhome page.php");
